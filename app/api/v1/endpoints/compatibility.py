...
from app.api.v1.schemas import CompatibilityRequest, CompatibilityResponse
...
from app.core.compatibility_service import compatibility_service
...
def analyze_compatibility(request: CompatibilityRequest):
    """
    Принимает JSON данные для анализа совместимости и возвращает результат анализа.
    
    Args:
        request: Запрос с JSON данными для анализа, флагом premium и моделью
    ...
    logger.info(f'Получен запрос на анализ совместимости. Premium: {request.premium}, Модель: {request.model}')
    
    try:
        interpretation = compatibility_service.get_interpretation(
            data=request.data,
            premium=request.premium,
            model=request.model
        )
        
        logger.info(f'Сгенерирован анализ совместимости длиной {len(interpretation)} символов.')
        return CompatibilityResponse(interpretation=interpretation)
        ...
        logger.error(f'Неожиданная внутренняя ошибка в эндпоинте /analyze: {e}', exc_info=True)
